...
class AdminUsersController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // 1. Generate strong 16-character random password on the server
        $plainPassword = \Illuminate\Support\Str::password(16, true, true, true, false);

        Log::info("Admin creating user: {$validated['email']}");

        // 2. Store ONLY the hashed password in the DB
        $user = User::create([
            'name' => $validated['name'],
            'email' => $validated['email'],
        ...
        // 3. Redirect with flashed data - Available ONLY for the next request
