        ...
        <div class="flex items-center gap-3">
            @if(Auth::user()->is_admin || Auth::user()->hasPermission('clients.create'))
                <a href="{{ route('clients.create') }}"
                    class="group flex items-center gap-2 px-6 py-2.5 bg-white text-violet-600 border border-violet-200 rounded-xl hover:bg-violet-50 transition-all duration-300 shadow-sm hover:-translate-y-0.5 font-semibold font-bold">
            ...
            @if(Auth::user()->is_admin || Auth::user()->hasPermission('documents.create'))
                <a href="{{ route('documents.create') }}"
                    class="group flex items-center gap-2 px-6 py-2.5 bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white rounded-xl hover:from-violet-700 hover:to-fuchsia-700 transition-all duration-300 shadow-lg shadow-violet-200 hover:-translate-y-0.5 font-semibold font-bold">
            ...
            <a href="{{ route('reclamations.index') }}"
                class="group flex items-center gap-2 px-5 py-2.5 bg-violet-50 text-violet-600 rounded-xl hover:bg-violet-100 transition-all duration-300 shadow-sm border border-violet-200 shrink-0 font-bold">
